...
function Comment({ postId }) {
    ...
    useEffect(() => {
        const fetchCmts = async () => {
            try {
                setLoading(true);
                const response = await axiosPublic.get(`posts/${postId}/comments`);
    ...
    const handlePostComment = async (e) => {
        ...
        try {
            // if edit mode
            if (cancel) {
                const res = await axiosPrivate.patch(`comments/${holdCmt.id}`, { message: comment });
                ...
                const res = await axiosPrivate.post(`/comments`, { message: comment, post_id: postId });
        ...
        <section className="bg-white py-8 antialiased">
            {!loading && (
                <div className="max-w-2xl mx-auto px-4">
                    ...
                    {comments.length === 0 && <div className="text-gray-600 text-center"> No comments yet.</div>}
                </div>
            )}
            {loading && (
               <SpinLoader/>

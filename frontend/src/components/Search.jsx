    ...
    useEffect(() => {
        ...
        const search = async () => {
            const [userResponse, postResponse] = await Promise.all([
                publicRequest.get(`/users`, { params: { q: searchValue } }),
                publicRequest.get(`/posts`, { params: { q: searchValue } }),
            ]);
            setUserSearchResult(userResponse.data);
            setPostSearchResult(postResponse.data);
        };
        search();
    }, [searchValue]);
                        ...
                        {userSearchResult && userSearchResult.length > 0 && (
                            <>
                                ...
                                {userSearchResult.map((user) => (
                                    <AccountItem key={user.id} user={user} onClick={() => setShowResults(false)} />
                                ))}
                            </>
                        )}
                        ...
                        {postSearchResult && postSearchResult.length > 0 && (
                            <>
                                ...
                                {postSearchResult.map((post) => (
                                    <PostItemSummary key={post.id} post={post} onClick={() => setShowResults(false)} />
                                ))}
                            </>
                        )}
                    </PoperWrapper>

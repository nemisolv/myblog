...
function PostDetail() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const postResponse = await axiosPublic.get(`posts/${slug}`);
                console.log('post response' + postResponse);

                const rePosts = await axiosPublic.get(
                    `posts/${postResponse.data.id}/related/${postResponse.data.tag.id}`,
                );
                setPost(postResponse.data);
                setRelatedPosts(rePosts.data);
                setLoading(false);
            } catch (error) {
                setLoading(false);
                                ...
                                >
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <p className="text-gray-600 text-xs">
                                            {post?.created_at && formatDistanceToNow(post.created_at) + ' ago'}
                                        </p>
                                    </div>
                                </div>
                            </div>

...
function Dashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const postCount = await privateRequest.get('/posts/count');
                setDashboardOverview({ postCount: postCount });
                const userCount = await privateRequest.get('/users/count');
                console.log('🚀 ~ fetchData ~ userCount:', userCount);
                setDashboardOverview((prev) => ({ ...prev, userCount: userCount }));
                const tagCount = await privateRequest.get('/tags/count');
                console.log('🚀 ~ fetchData ~ tagCount:', tagCount);
                setDashboardOverview((prev) => ({ ...prev, tagCount: tagCount }));
            } catch (error) {
                console.log(error);
            }
        };
        ...
        const fetchRecentPosts = async () => {
            const res = await privateRequest.get('posts/recent');
            setRecentPosts(res);
        };

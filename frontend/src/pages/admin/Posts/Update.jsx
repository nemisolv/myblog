...
function UpdatePost() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            setLoading(true);
            const res = await privateRequest.put(`posts/${post.id}`, formData);
            setLoading(false);
            if (res.status === 200) {
    ...
    useEffect(() => {
        async function fetchData() {
            const post = await privateRequest.get(`posts/id/${id}`);
            reset(post);
            
            setPost(post);
            setImage({ src: post.thumbnail });
            setSelectTag(post?.tag || '');
            setContent(post?.content || '');
            setValue('tagId', post.tag.id);
        }
        fetchData();
